...
//    save：  不会去重，保存所有数据
     ...
     * @param bean
     */
    public static void insertData(VideoDBBean bean) {
     ...
     * @param bean
     */
    public static void insertOrReplaceData(VideoDBBean bean) {
    ...
    public static void deleteData(VideoDBBean bean) {
    ...
    public static void updateData(VideoDBBean bean) {
    ...
    public static List queryRaw(Long id) {
        ...
        List<VideoDBBean> beanLis = (List<VideoDBBean>)
                App.getInstance().getDaoSession().queryRaw(VideoDBBean.class, " where id = ?", id + "");
        return beanLis;
    }
    public static List queryRawTag(String tag) {

        List<VideoDBBean> beanLis = (List<VideoDBBean>)
                App.getInstance().getDaoSession().queryRaw(VideoDBBean.class, " where tag = ?", tag + "");
        return beanLis;
    }
