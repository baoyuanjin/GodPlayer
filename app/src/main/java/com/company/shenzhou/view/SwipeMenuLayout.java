 ...
 * <p/>
 * fix 【多指一起滑我的情况】：只接第一个客人(使用一个类静态布尔变量)
 * other:
 * 1 菜单处于侧滑时，拦截长按事件
 * 2 解决侧滑时 点击 的冲突
 * 3 通过 isIos 变量控制是否是IOS阻塞式交互，默认是打开的。
 * 4 通过 isSwipeEnable 变量控制是否开启右滑菜单，默认打开。（某些场景，复用item，没有编辑权限的用户不能右滑）
 * 5 add，通过开关 isLeftSwipe支持左滑右滑
 * 6 add , 增加viewChache 的 get()方法，可以用在：当点击外部空白处时，关闭正在展开的侧滑菜单。
 * 7  fix , 当父控件宽度不是全屏时的bug。
 * add , 仿QQ，侧滑菜单展开时，点击除侧滑菜单之外的区域，关闭侧滑菜单。
 * 8  add,判断手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件。
 * 9  fix 长按事件和侧滑的冲突。
 * 10  add,适配GridLayoutManager，将以第一个子Item(即ContentItem)的宽度为控件宽度。
 * 11 add,支持padding,且后续计划加入上滑下滑，因此不再支持ContentItem的margin属性。
 *  add,修改回弹的动画，更平滑。
 *  fix,微小位移的move不回回弹的bug
 * fix 当ItemView存在高度可变的情况
 * fix 禁止侧滑时(isSwipeEnable false)，点击事件不受干扰。
 * fix ListView快速滑动快速删除时，偶现菜单不消失的bug。
 *
 */
